...
def bb8_transform(ex_rois, gt_bb8_coordinates, bb8_variance, granularity, im_info):
    ...
    FGA_reg_targets_dx = FGA_reg_targets_dx / (ex_widths[:, np.newaxis] + 1e-14) / bb8_variance[0]
    FGA_reg_targets_dy = distance_y[FGA_cls_targets > 0].reshape((distance_y.shape[0], -1))
    FGA_reg_targets_dy = FGA_reg_targets_dy / (ex_heights[:, np.newaxis] + 1e-14) / bb8_variance[1]
