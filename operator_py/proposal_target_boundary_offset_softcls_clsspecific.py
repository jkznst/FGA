...
def bb8_transform_cls_specific(ex_rois, cid_labels, gt_bb8_coordinates, bb8_variance, im_info, num_classes):
    """
    compute bounding box regression targets from ex_rois to gt_rois
    :param ex_rois: [N, 4]
    :param cid_labels: [N, ]
    ...
    # cls specific implementation
    cids_mask = np.eye(num_classes)[cid_labels.reshape(-1)]  # (N, 8)
    cls_target_cids_mask = np.repeat(cids_mask, repeats=32, axis=-1).reshape((cid_labels.shape[0], -1, 4))  # (N, 64, 4)

    boundary_cls_targets_cls_specific = np.tile(boundary_cls_targets, reps=(1, num_classes, 1))  # (N, 64, 4)
    boundary_cls_targets_cls_specific = np.where(cls_target_cids_mask, boundary_cls_targets_cls_specific,
                                                 -1 * np.ones_like(boundary_cls_targets_cls_specific))
    boundary_reg_targets_cls_specific = np.tile(boundary_reg_targets, reps=(1, num_classes))
    boundary_reg_weights_cls_specific = np.tile(boundary_reg_weights, reps=(1, num_classes)) * np.repeat(cids_mask, repeats=64, axis=-1)

    # print(cid_labels[0:5])
    # print(boundary_cls_targets_cls_specific[0, 0:9])
    # print(boundary_reg_targets_cls_specific[0])
    # print(boundary_reg_weights_cls_specific[0])

    return boundary_cls_targets_cls_specific, boundary_reg_targets_cls_specific, boundary_reg_weights_cls_specific


def sample_rois_cls_specific(rois, gt_boxes, num_classes, rois_per_image, fg_rois_per_image, fg_overlap, bb8_variance, im_info):
    """
    ...
    cid_labels = gt_boxes[gt_assignment, 0].astype(np.int)
    ...
    boundary_cls_targets, boundary_reg_targets, boundary_reg_weights = \
        bb8_transform_cls_specific(rois[:, 1:], cid_labels, gt_boxes[gt_assignment[keep_indexes], 8:24],
                      bb8_variance=bb8_variance, im_info=im_info, num_classes=num_classes)
...
class BB8ProposalTargetOperator(mx.operator.CustomOp):
    def __init__(self, num_keypoints, batch_images, batch_rois, fg_fraction, fg_overlap, bb8_variance, im_info, num_classes):
    ...
    def forward(self, is_train, req, in_data, out_data, aux):
        ...
        bb8_boundary_cls_target = np.empty((0, self._num_classes * self._num_keypoints, 4), dtype=np.float32)
        bb8_boundary_reg_target = np.empty((0, self._num_classes * 2 * self._num_keypoints * 4), dtype=np.float32)
        bb8_boundary_reg_weight = np.empty((0, self._num_classes * 2 * self._num_keypoints * 4), dtype=np.float32)
        for batch_idx in range(self._batch_images):
            ...
            b_rois, b_bb8_boundary_cls_target, b_bb8_boundary_reg_target, b_bb8_boundary_reg_weight = \
                sample_rois_cls_specific(b_rois, b_gt_boxes, num_classes=self._num_classes, rois_per_image=self._rois_per_image,
...
@mx.operator.register('bb8_proposal_target_boundary_offset_softcls_clsspecific')
class BB8ProposalTargetProp(mx.operator.CustomOpProp):
    def __init__(self, num_keypoints='8', batch_images='1', batch_rois='128', fg_fraction='0.25',
                 fg_overlap='0.5', bb8_variance='(0.1, 0.1)', im_info='(512, 512, 1)', num_classes='8'):
    ...
    def infer_shape(self, in_shape):
        ...
        bb8_boundary_cls_target_shape = (self._batch_rois, self._num_classes * self._num_keypoints, 4)
        ...
        bb8_boundary_reg_target_shape = (self._batch_rois, self._num_classes * self._num_keypoints * 2 * 4)
        bb8_boundary_reg_weight_shape = (self._batch_rois, self._num_classes * self._num_keypoints * 2 * 4)
    ...
    def create_operator(self, ctx, shapes, dtypes):
        return BB8ProposalTargetOperator(self._num_keypoints, self._batch_images, self._batch_rois, self._fg_fraction,
                                      self._fg_overlap, self._bb8_variance, self._im_info, self._num_classes)
