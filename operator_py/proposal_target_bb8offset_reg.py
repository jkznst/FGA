...
def bb8_transform(ex_rois, gt_bb8_coordinates, bb8_variance, im_info):
    ...
    ex_widths = ex_rois[:, 2] - ex_rois[:, 0] + 1.0
    ex_heights = ex_rois[:, 3] - ex_rois[:, 1] + 1.0
    ex_ctr_x = ex_rois[:, 0] + 0.5 * (ex_widths - 1.0)    # shape [N,]
    ex_ctr_y = ex_rois[:, 1] + 0.5 * (ex_heights - 1.0)

    gt_bb8_coordinates = gt_bb8_coordinates.reshape((gt_bb8_coordinates.shape[0], 8, 2))
    gt_bb8_coordinates_x = gt_bb8_coordinates[:, :, 0] * im_info[1]    # shape [N, 8]
    gt_bb8_coordinates_y = gt_bb8_coordinates[:, :, 1] * im_info[0]

    distance_x = gt_bb8_coordinates_x - ex_ctr_x[:, np.newaxis]    # shape [N, 8]
    distance_y = gt_bb8_coordinates_y - ex_ctr_y[:, np.newaxis]

    FGA_reg_targets_dx = distance_x / (ex_widths[:, np.newaxis] + 1e-14) / bb8_variance[0]
    FGA_reg_targets_dy = distance_y / (ex_heights[:, np.newaxis] + 1e-14) / bb8_variance[1]

    # shape [N, 8, 2]
    FGA_reg_targets = np.stack((FGA_reg_targets_dx, FGA_reg_targets_dy), axis=2)
    # shape [N, 16] [xyxy]
    FGA_reg_targets = FGA_reg_targets.reshape((FGA_reg_targets.shape[0], -1))
    FGA_reg_weights = np.ones_like(FGA_reg_targets)

    return FGA_reg_targets, FGA_reg_weights
...
def sample_rois(rois, gt_boxes, num_classes, rois_per_image, fg_rois_per_image, fg_overlap, bb8_variance, im_info):
    ...
    FGA_reg_targets, FGA_reg_weights = \
    ...
    return rois, FGA_reg_targets, FGA_reg_weights
...
class BB8ProposalTargetOperator(mx.operator.CustomOp):
    ...
    def forward(self, is_train, req, in_data, out_data, aux):
        ...
        for batch_idx in range(self._batch_images):
            ...
            b_rois, b_bb8_FGA_reg_target, b_bb8_FGA_reg_weight = \
        ...
        self.assign(out_data[1], req[1], bb8_FGA_reg_target)
        self.assign(out_data[2], req[2], bb8_FGA_reg_weight)
...
@mx.operator.register('bb8_proposal_target_offset_reg')
class BB8ProposalTargetProp(mx.operator.CustomOpProp):
    ...
    def list_outputs(self):
        return ['rois_output', 'bb8_offset_reg_target', 'bb8_offset_reg_weight']
    ...
    def infer_shape(self, in_shape):
        ...
        bb8_offset_reg_target_shape = (self._batch_rois, self._num_keypoints * 2)
        bb8_offset_reg_weight_shape = (self._batch_rois, self._num_keypoints * 2)

        return [rpn_rois_shape, gt_boxes_shape], \
               [output_rois_shape, bb8_offset_reg_target_shape, bb8_offset_reg_weight_shape]
