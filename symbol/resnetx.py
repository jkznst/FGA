...
def inception_pooling(data, num_filter, name):
    pool0 = mx.symbol.Pooling(data=data, kernel=(3, 3), stride=(2, 2), pad=(1, 1), pool_type='max', name=name + '_pool0')

    tower1_bn1 = mx.sym.BatchNorm(data=data, fix_gamma=False, eps=2e-5, momentum=0.9, name=name + '_tower1_bn1')
    tower1_relu1 = mx.sym.Activation(data=tower1_bn1, act_type='relu', name=name + '_tower1_relu1')
    tower1_conv1 = mx.sym.Convolution(data=tower1_relu1, num_filter=int(num_filter), kernel=(1,1), stride=(1,1), pad=(0,0),
                                   no_bias=True, workspace=workspace, name=name + '_tower1_conv1')
    tower1_bn2 = mx.sym.BatchNorm(data=tower1_conv1, fix_gamma=False, eps=2e-5, momentum=0.9, name=name + '_tower1_bn2')
    tower1_relu2 = mx.sym.Activation(data=tower1_bn2, act_type='relu', name=name + '_tower1_relu2')
    tower1_conv2 = mx.sym.Convolution(data=tower1_relu2, num_filter=int(num_filter), kernel=(3, 3), stride=(2, 2), pad=(1, 1),
                                no_bias=True, workspace=workspace, name=name + '_tower1_conv2')

    tower2_bn1 = mx.sym.BatchNorm(data=data, fix_gamma=False, eps=2e-5, momentum=0.9, name=name + '_tower2_bn1')
    tower2_relu1 = mx.sym.Activation(data=tower2_bn1, act_type='relu', name=name + '_tower2_relu1')
    tower2_conv1 = mx.sym.Convolution(data=tower2_relu1, num_filter=int(num_filter), kernel=(1, 1), stride=(1, 1), pad=(0, 0),
                                no_bias=True, workspace=workspace, name=name + '_tower2_conv1')
    tower2_bn2 = mx.sym.BatchNorm(data=tower2_conv1, fix_gamma=False, eps=2e-5, momentum=0.9, name=name + '_tower2_bn2')
    tower2_relu2 = mx.sym.Activation(data=tower2_bn2, act_type='relu', name=name + '_tower2_relu2')
    tower2_conv2 = mx.sym.Convolution(data=tower2_relu2, num_filter=int(num_filter), kernel=(3, 3), stride=(1, 1), pad=(1, 1),
                                no_bias=True, workspace=workspace, name=name + '_tower2_conv2')
    tower2_bn3 = mx.sym.BatchNorm(data=tower2_conv2, fix_gamma=False, eps=2e-5, momentum=0.9, name=name + '_tower2_bn3')
    tower2_relu3 = mx.sym.Activation(data=tower2_bn3, act_type='relu', name=name + '_tower2_relu3')
    tower2_conv3 = mx.sym.Convolution(data=tower2_relu3, num_filter=int(num_filter), kernel=(3, 3), stride=(2, 2), pad=(1, 1),
                                no_bias=True, workspace=workspace, name=name + '_tower2_conv3')

    concat = mx.symbol.concat(pool0, tower1_conv2, tower2_conv3, dim=1)
    concat_bn1 = mx.sym.BatchNorm(data=concat, fix_gamma=False, eps=2e-5, momentum=0.9, name=name + '_concat_bn1')
    concat_relu1 = mx.sym.Activation(data=concat_bn1, act_type='relu', name=name + '_concat_relu1')
    concat_conv1 = mx.sym.Convolution(data=concat_relu1, num_filter=int(num_filter), kernel=(1, 1), stride=(1, 1),
                                      pad=(0, 0),
                                      no_bias=True, workspace=workspace, name=name + '_concat_conv1')
    return concat_conv1
...
def get_resnetx_conv(data, num_layers):
    ...
    pool0 = inception_pooling(data=relu0, num_filter=64, name="inception_pool0")
...
def get_ssd_conv(data, num_layers):
    conv_C5, conv_C4, conv_C3, _ = get_resnetx_conv(data, num_layers)
