...
def multibox_layer_FPN_RCNN(from_layers, num_classes, sizes=[.2, .95],
                    ratios=[1], normalization=-1, num_channels=[],
                    clip=False, interm_layer=0, steps=[]):
    """
    the basic aggregation module for SSD detection. Takes in multiple layers,
    ...
    cls_pred_layers = []
    anchor_layers = []
    loc_pred_layers_dict = {}
    cls_prob_layers_dict = {}
    anchor_layers_dict = {}
    ...
    for k, from_layer in enumerate(from_layers):
        ...
        #                                           shape=(0, 0, -1, num_classes),
        #                                           name="{}_rpn_cls_score_reshape".format(from_name))
        # rpn_cls_prob = mx.symbol.softmax(data=rpn_cls_score_reshape,
        #                                           axis=-1,
        #                                            name="{}_rpn_cls_prob".format(from_name))
        # rpn_cls_prob_reshape = mx.symbol.Reshape(data=rpn_cls_prob,
        #                                          shape=(0, 0, -1, num_cls_pred),
        #                                          name='{}_rpn_cls_prob_reshape'.format(from_name))
        # rpn_cls_prob_reshape = rpn_cls_prob_reshape.transpose((0,3,1,2))
        # cls_prob_layers_dict.update({'cls_prob_stride%s' % feat_stride[k]: rpn_cls_prob_reshape})
...
def multibox_layer_SSD_RCNN(from_layers, num_classes, sizes=[.2, .95],
                    ratios=[1], normalization=-1, num_channels=[],
                    clip=False, interm_layer=0, steps=[]):
    """
    the basic aggregation module for SSD detection. Takes in multiple layers,
    ...
    Returns:
    ----------
    list of outputs, as [loc_pred_layers_for_proposal, cls_pred_layers_for_proposal, anchor_layers_for_proposal,
                        loc_preds, cls_preds, anchor_boxes]
    The first three are used for region proposal
    loc_pred_layers_for_proposal: list of [N, num_anchors * 4, H, W]
    cls_pred_layers_for_proposal: list of [N, num_anchors * num_class, H, W]
    anchor_layers_for_proposal: list of [N, num_all_anchors, 4]
    The last three are used for calculating losses
    loc_preds : localization regression prediction
    cls_preds : classification prediction
    anchor_boxes : generated anchor boxes
    """
    ...
    for k, from_layer in enumerate(from_layers):
        ...
        cls_pred = mx.symbol.Convolution(data=from_layer, bias=bias, kernel=(3,3), \
            ...
            name="{}_cls_pred_conv".format(from_name))
        rpn_cls_score_reshape = mx.symbol.Reshape(data=cls_pred,
                                                  shape=(0, num_classes, -1, 0),
                                                  name="rpn_cls_score_reshape")
        rpn_cls_prob = mx.symbol.SoftmaxActivation(data=rpn_cls_score_reshape,
                                                   mode="channel",
                                                   name="rpn_cls_prob_stride%s" % feat_stride[k])
        rpn_cls_prob_reshape = mx.symbol.Reshape(data=rpn_cls_prob,
                                                 shape=(0, num_cls_pred, -1, 0),
        ...
        anchors = mx.contrib.symbol.MultiBoxPrior(from_layer, sizes=size_str, ratios=ratio_str, \
            clip=clip, name="{}_anchors".format(from_name), steps=step)
        anchor_layers_dict.update({'rpn_anchor_stride%s' % feat_stride[k]: anchors})
        anchors = mx.symbol.Flatten(data=anchors)
        anchor_layers.append(anchors)
