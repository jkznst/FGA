...
def get_RCNN_resnetm_fpn_train(num_classes, alpha_bb8, num_layers, num_filters,
    ...
    cls_prob = mx.symbol.SoftmaxOutput(data=cls_preds, label=cls_target, \
        ignore_label=-1, use_ignore=True, grad_scale=0.5, multi_output=True, \
        normalization='valid', name="cls_prob")
    loc_loss_ = mx.symbol.smooth_l1(name="loc_loss_", \
        data=loc_target_mask * (loc_preds - loc_target), scalar=1.0)
    ...
    rpn_det = mx.contrib.symbol.MultiBoxDetection(*[cls_prob, loc_preds, anchor_boxes], \
        name="rpn_proposal", nms_threshold=0.8, force_suppress=False,
        variances=(0.1, 0.1, 0.2, 0.2), nms_topk=400)
    ...
    # rcnn roi proposal target
    group = mx.symbol.Custom(rois=rois, gt_boxes=label, op_type='bb8_proposal_target_heatmap',
                         num_keypoints=8, batch_images=2,
    ...
    head_conv1 = mx.symbol.Convolution(data=roi_pool, weight=rcnn_head_conv_weight, no_bias=False,
                                      kernel=(1, 1), stride=(1,1), pad=(0,0), num_filter=256, name="rcnn_head_conv1")
    # head_bn1 = mx.symbol.BatchNorm(head_conv1, name="rcnn_head_bn1")
    head_relu1 = mx.symbol.Activation(head_conv1, act_type='relu', name="rcnn_head_relu1")

    rcnn_FGA_cls_score = mx.symbol.Convolution(data=head_relu1, no_bias=False,
                                      kernel=(3, 3), stride=(1, 1), pad=(1, 1), num_filter=8)
    rcnn_FGA_cls_score = mx.symbol.contrib.BilinearResize2D(data=rcnn_FGA_cls_score, height=granularity[0], width=granularity[1], name="rcnn_FGA_cls_score")
    # rcnn_FGA_cls_score = mx.symbol.BatchNorm(rcnn_FGA_cls_score, name='rcnn_FGA_cls_score')
    # head_relu2 = mx.symbol.Activation(head_conv2, act_type='relu', name="rcnn_head_relu2")

    # rcnn_FGA_cls_score = mx.symbol.Convolution(data=head_relu1, weight=rcnn_FGA_cls_weight, bias=rcnn_FGA_cls_bias,
    #                                            kernel=(1, 1), stride=(1, 1), pad=(0, 0), num_filter=8)
    # rcnn_FGA_bb8_pred = mx.symbol.Convolution(data=head_relu1, weight=rcnn_FGA_reg_weight, no_bias=False,
    #                                           kernel=(3, 3), stride=(1, 1), pad=(1, 1),
    #                                           num_filter=16)
    # rcnn_FGA_bb8_pred = mx.symbol.contrib.BilinearResize2D(rcnn_FGA_bb8_pred, height=granularity[0], width=granularity[1], name='rcnn_FGA_bb8_pred')
    # rcnn_FGA_bb8_pred = mx.symbol.BatchNorm(rcnn_FGA_bb8_pred, name='rcnn_FGA_bb8_pred')
    ...
    rcnn_FGA_bb8_pred = mx.symbol.FullyConnected(data=relu7, num_hidden=16, name='rcnn_FGA_bb8_pred')

    # loss
    # rcnn_FGA_cls_prob = mx.symbol.SoftmaxOutput(data=rcnn_FGA_cls_score.reshape((0,8,-1)), label=FGA_cls_target, \
    #     ignore_label=-1, use_ignore=True, grad_scale=2., preserve_shape=True, \
    #     normalization='valid', name="rcnn_FGA_cls_prob")
        # mx.symbol.LogisticRegressionOutput(data=rcnn_FGA_cls_score, label=FGA_cls_target,
        #                                grad_scale=1.0 / 640., name='rcnn_FGA_cls_prob')
    ...
    # out = mx.symbol.Group([cls_prob, loc_loss, cls_label, det])
    out = mx.symbol.Group([cls_prob, loc_loss, cls_label, det, *rcnn_group, rcnn_FGA_cls_target,
                           rcnn_FGA_reg_target, rpn_loc_target,
                           rois, score, cid, rcnn_FGA_cls_score, rcnn_FGA_bb8_pred])
    return out
...
def get_RCNN_resnetm_fpn_test(num_classes, num_layers, num_filters,
    ...
    # head_conv = mx.symbol.Convolution(data=roi_pool, weight=rcnn_head_conv_weight, bias=rcnn_head_conv_bias,
    #                                   kernel=(3, 3), stride=(1,1), pad=(1,1), num_filter=256, name="rcnn_head_conv")
    # head_relu = mx.symbol.Activation(head_conv, act_type='relu', name="rcnn_head_relu")
    #
    # rcnn_cls_score = mx.symbol.Convolution(data=head_relu, weight=rcnn_FGA_cls_weight, bias=rcnn_FGA_cls_bias,
    #                                        kernel=(3, 3), stride=(1,1), pad=(1,1), num_filter=8, name="rcnn_FGA_cls_score")
    # rcnn_FGA_cls_score = mx.symbol.sigmoid(rcnn_cls_score, name="rcnn_FGA_cls_prob")
    # rcnn_FGA_bb8_pred = mx.symbol.Convolution(data=head_relu, weight=rcnn_FGA_reg_weight, bias=rcnn_FGA_reg_bias,
    #                                       kernel=(3, 3), stride=(1, 1), pad=(1, 1),
    #                                        num_filter=16, name="rcnn_FGA_bb8_pred")
    ...
    # # classification
    # rcnn_FGA_cls_score = mx.symbol.FullyConnected(data=relu7, num_hidden=8 * granularity[0] * granularity[1])
    # rcnn_FGA_cls_score = mx.symbol.softmax(data=rcnn_FGA_cls_score.reshape((0, 8, -1)), axis=2)
    # rcnn_FGA_cls_score = mx.symbol.reshape(rcnn_FGA_cls_score, shape=(0, 8, granularity[0], granularity[1]), name='rcnn_FGA_cls_prob')
    # # bounding box regression
    # rcnn_FGA_bb8_pred = mx.symbol.FullyConnected(data=relu7, num_hidden=16 * granularity[0] * granularity[1])
    # rcnn_FGA_bb8_pred = mx.symbol.reshape(rcnn_FGA_bb8_pred, shape=(0, 16, granularity[0], granularity[1]), name="rcnn_FGA_bb8_pred")
    # concat output of each level
    # rois_concat = mx.symbol.concat(*rois_list, dim=1)
    # score_concat = mx.symbol.concat(*score_list, dim=1)
    # cid_concat = mx.symbol.concat(*cid_list, dim=1)
    # rcnn_FGA_cls_score_concat = mx.symbol.concat(*rcnn_cls_score_list, dim=0)  # [num_rois_4level, num_keypoints, granularity[0], granularity[1]]
    # rcnn_FGA_bb8_pred_concat = mx.symbol.concat(*rcnn_bb8_pred_list, dim=0)  # [num_rois_4level, num_keypoints*2, granularity[0], granularity[1]]
    ...
    rcnn_FGA_cls_score = mx.symbol.Convolution(data=head_relu1, no_bias=False,
                                               kernel=(3, 3), stride=(1, 1), pad=(1, 1), num_filter=8)
    rcnn_FGA_cls_score = mx.symbol.contrib.BilinearResize2D(data=rcnn_FGA_cls_score, height=granularity[0],
                                                            width=granularity[1], name="rcnn_FGA_cls_score")
    ...
    out = mx.symbol.Group([rois, score, cid, rcnn_FGA_cls_score, rcnn_FGA_bb8_pred])
    return out
